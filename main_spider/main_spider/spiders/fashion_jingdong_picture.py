...
class TaobaoFasSpider(Spider):
    name = 'fashion_jingdong_picture'
    allowed_domains = ['search.jd.com','detail.tmall.com']
    ...
    def __init__(self):
        ...
        self.seach_url = 'https://search.jd.com/s_new.php?keyword={0}&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq={0}&page={1}&s={2}&click=0'
        self.headers = {
            ':authority':'search.jd.com',
            'referer':'https://search.jd.com/Search?keyword=%E8%A3%99%E5%AD%90&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq=%E8%A3%99%E5%AD%90&page=1&s=1&click=0',
            'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.139 Safari/537.36'
        }
        self.merge_url = 'https://item.jd.com'
        self.next_url = 'https://search.jd.com/s_new.php?keyword={0}&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&wq={0}&page={1}&s={2}&scrolling=y&tpl=3_L&show_items={3}'
    ...
    def start_requests(self):
        ...
        for i in range(1,4):
            ...
            url = self.seach_url.format(keywords,page,s)
            yield scrapy.Request(url,callback=self.parse_item,headers=headers,meta={'page':page,'s':s,'keywords':keywords,'flag':True})
            s += index_s
            page += index_page
            referer = url
    ...
    def parse_item(self,response):
        search_nodes = response.xpath('//li[@class="gl-item"]')
        for node in search_nodes:
            img_url_list = []
            title = node.xpath('.//div[@class="p-img"]/a/@title').extract_first()
            url = urljoin(self.merge_url,node.xpath('.//div[@class="p-img"]/a/@href').extract_first())
            img_url = urljoin(self.merge_url,node.xpath('.//div[@class="p-img"]/a/img/@source-data-lazy-img').extract_first())
            if img_url:
        ...
        if response.meta['flag']:
            next_item_ids = response.xpath('//li[@class="gl-item"]/@data-pid').extract()
            if next_item_ids != []:
                keywords = response.meta['keywords']
                page = response.meta['page']
                s = response.meta['s']
                item_ids = ','.join(next_item_ids)
                next_url = self.next_url.format(keywords,page+1,s+25,item_ids)
                headers = {
                    ':authority': 'search.jd.com',
                    'referer': response.url,
                    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.139 Safari/537.36'
                }
                yield scrapy.Request(next_url,callback=self.parse_item,headers=headers,meta={'flag':False})
